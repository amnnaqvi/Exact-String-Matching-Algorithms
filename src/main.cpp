...
//  reporting style.
//
//  FBAS paper fixed patterns (Garraoui 2025, Table II):
//    For the Dante corpus we ALWAYS benchmark the exact 12 patterns
//    used in the FBAS paper so our results can be placed directly
//    next to their Table II.  These are added FIRST, before the
//    random length-sweep sample, and are tagged rarity_bucket =
//    "paper_fixed" so analyse.py can isolate them easily.
//    Patterns that do not appear in our normalised Dante text are
//    skipped with a warning rather than crashing.
// ================================================================

static constexpr size_t PATTERNS_PER_LENGTH = 20;

// The 12 patterns from Garraoui (2025) Table II, in paper order.
...
// Helper: return true if 'pattern' appears at least once in 'text'
// (case-sensitive, exact).  Uses std::string::find for simplicity.
...
int main() {
    ...
    for (const auto& corpus : corpora) {
        ...
        // ----------------------------------------------------------------
        //  For the Dante corpus: inject the 12 FBAS paper patterns first.
        //  Tag them "paper_fixed" so analyse.py can treat them separately
        //  from the random length-sweep sample.
        // ----------------------------------------------------------------
        if (corpus.name == "dante") {
            std::cout << "  Adding FBAS paper fixed patterns (Garraoui 2025)...\n";
            int added = 0, skipped = 0;
            for (const auto& pat : FBAS_PAPER_PATTERNS) {
                if (exists_in_text(text, pat)) {
                    // Use "paper_fixed" as the rarity bucket so they appear
                    // as a separate group in the replication figures.
                    patterns.push_back({pat, "paper_fixed"});
                    ++added;
                } else {
                    std::cerr << "  [WARN] Paper pattern not found in Dante text: \""
                              << pat << "\" — skipping.\n";
                    ++skipped;
                }
            }
        ...
        // ----------------------------------------------------------------
        //  Random length-sweep sample (same for all corpora).
        // ----------------------------------------------------------------
        PatternSampler sampler(42);
